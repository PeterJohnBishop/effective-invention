...
func RegisterUser(db *sql.DB, c *gin.Context) {
	...
	access, refresh, err := jwt.GenerateTokens(userId)
	if err != nil {
	...
	jwt.StoreTokens(userId, access, refresh)
...
func Login(db *sql.DB, c *gin.Context) {
	...
	var user User
	query := `SELECT id, name, email, password, created, updated FROM users WHERE email = $1`
	...
	jwt.StoreTokens(user.ID, access, refresh)
...
func Refresh(c *gin.Context) {
	...
	claims, err := jwt.ValidateToken(body.RefreshToken, true)
	if err != nil {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "Invalid refresh token"})
		return
	}

	newAccess, _, err := jwt.GenerateTokens(claims.ID)
	if err != nil {
...
func GetUsers(db *sql.DB, c *gin.Context) {
	...
	rows, err := db.QueryContext(c, "SELECT id, name, email, password, created, updated FROM users;")
	if err != nil {
...
func GetUserByID(db *sql.DB, c *gin.Context) {
	...
	var user User
	query := `SELECT id, name, email, password, created, updated FROM users WHERE id = $1`
...
func UpdateUser(db *sql.DB, c *gin.Context) {
	...
	query := `UPDATE users SET name=$1, email=$2, updated=EXTRACT(EPOCH FROM now()) WHERE id=$3`
